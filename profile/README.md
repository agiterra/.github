...
Small, composable, generic — the Unix philosophy, applied to AI agents. Each tool does one thing well and stands on its own; together they're dogfooded daily into a coordinated, persistent team. Every piece is AI-vendor-agnostic (Claude Code, Codex, or your own runtime), and each capability has been tuned for performance over six months of real multi-agent work.
...
| [crew](https://github.com/agiterra/crew-claude-code) | **Persistent orchestration.** Agents run in `screen` sessions that outlive a closed laptop, a killed terminal, or a reboot. Steer any agent directly, read its output, run it headless, arrange them across themed panes and tabs. Identity is provisioned by env-forwarding — keys never touch disk. |
| [crew-fleet](https://github.com/agiterra/crew-fleet-claude-code) | **Your whole fleet in one query.** SSH fan-out unions agents from every registered machine into a single view — no central state, no sync daemon. Unreachable machines surface in their own list with the reason; they never fail the call. |
...
| [seance](https://github.com/agiterra/seance-claude-code) · [codex](https://github.com/agiterra/seance-codex) | **Borrow a persona.** Summon another personai's identity and knowledge into your session — across Claude Code or Codex — without spawning a whole new agent. |
